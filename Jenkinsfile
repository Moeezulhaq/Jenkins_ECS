pipeline {
  ...
  stages { 
        ...
        steps {
        sh "aws ecs register-task-definition --cli-input-json file://taskdenfi.json --region us-east-1"    
        }
    }    
    ...
    // // }
        ...
        steps {
        sh " aws ecs create-service --cluster mycluster --service-name myservice --task-definition flask-signup --load-balancers loadBalancerName=ecs-alb-1890033726,containerName=app,containerPort=80 --role ecsServiceRole --desired-count 0"    
        }
    }
