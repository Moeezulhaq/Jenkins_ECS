...
//     )
  ...
  stages { 
        ...
        steps {
        sh "aws ecs create-service --cluster mycluster --service-name MyService --desired-count 2  --network-configuration awsvpcConfiguration={subnets=[subnet-0b48f8acbc06080d4],securityGroups=[sg-0541867ccac96203a],assignPublicIp=ENABLED} --cli-input-json file://service.defination.json --region us-east-1"    
        }
    }
