...
//   }
  ...
  stages { 
        ...
        steps {
        sh "docker login -u AWS -p $(aws ecr get-login-password --region us-east-1 | docker login --username AWS --password-stdin 489994096722.dkr.ecr.us-east-1.amazonaws.com"
        }
    }
