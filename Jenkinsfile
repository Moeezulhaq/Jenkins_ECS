pipeline {
  ...
  stages { 
    ...
    // // }
        ...
        steps {
        sh " aws ecs create-service --cluster mycluster --service-name myservice --task-definition flask-signup --load-balancers loadBalancerName=ecs-alb,containerName=app,containerPort=80 --role ecsServiceRole --desired-count 0"    
        }
    }
