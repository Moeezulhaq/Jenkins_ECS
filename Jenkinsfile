pipeline {
  ...
  stages { 
        ...
        steps {
        sh "aws ecs create-service --cluster mycluster --service-name MyService --load-balancers loadBalancerName= --desired-count 2 --cli-input-json file://service.defination.json --region us-east-1"    
        }
    }
